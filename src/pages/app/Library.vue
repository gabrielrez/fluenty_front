...
<template>
    ...
    <div class="flex items-center gap-3 my-8">
        ...
        <button @click="showFilters = !showFilters"
            class="h-10 px-4 rounded-xl text-sm font-medium flex items-center gap-2 border cursor-pointer transition-all" :class="hasActiveFilters
        ...
        <button v-if="hasActiveFilters" @click="clearFilters"
            class="h-10 px-3 border rounded-xl flex items-center gap-2 text-sm bg-[#DD3C3C] text-white cursor-pointer transition-all">
            <X class="w-4 h-4" />
            Clear
        </button>
    </div>

    <div v-if="showFilters" class="border border-[#E0E5EE] rounded-2xl p-6 mb-10 space-y-6">
        <div>
            ...
            <div class="flex flex-wrap gap-2">
                <button v-for="level in levels" :key="level.value" @click="toggleLevel(level.value)"
                    class="px-4 py-2 rounded-lg text-sm border transition cursor-pointer" :class="{
        ...
        <div>
            ...
            <div class="flex flex-wrap gap-2">
                <button v-for="category in categories" :key="category.id" @click="toggleCategory(category.id)"
                    class="px-4 py-2 rounded-lg text-sm border transition cursor-pointer" :class="filters.category === category.id
